...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_initializer([n_features * embed_size, hidden_size]), name="W")
        b1 = tf.Variable(tf.zeros(shape=[hidden_size]), name="b1")
        U = tf.Variable(xavier_initializer([hidden_size, n_classes]), name="U")
    ...
    def add_loss_op(self, pred):
        ...
        """
        cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, 
                logits=pred, name="softmax_ce_with_logits")
