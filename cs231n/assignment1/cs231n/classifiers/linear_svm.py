...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # loss.                                                                     #
  #############################################################################
  truth_table = np.array(threshold != 0.0, dtype=np.float32) #C,N
  count = np.sum(truth_table, axis=0) - 1
  truth_table[y, np.arange(num_train)] = -count
  dW = truth_table.dot(X).T 
  dW /= float(num_train)
  dW += reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
